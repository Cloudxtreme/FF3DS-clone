...
static void boundaries(int c) {
	...
	if(sprites[c].x < (MIN_WIDTH << 8))  {
		sprites[c].x = (MAX_WIDTH << 8);
	}
	if(sprites[c].x > (MAX_WIDTH << 8)) {
		sprites[c].x = (MIN_WIDTH << 8);
	}
		
	if(sprites[c].y > (MAX_HEIGHT << 8)) {
		sprites[c].y = (MIN_HEIGHT << 8);
	}
	if(sprites[c].y < (MIN_HEIGHT << 8)) {
		sprites[c].y = (MAX_HEIGHT << 8);
	}

}
//---------------------------------------------------------------------------------
static bool moveSprites() {
//---------------------------------------------------------------------------------
	hidScanInput();
	u32 kHeld = hidKeysHeld();
	int c = 0; //which character the player is controlling
	if(kHeld) {
		printf("X: %i Y: %i\n", sprites[c].x, sprites[c].y);
	}
	if(kHeld & KEY_RIGHT) 
		sprites[c].x += sprites[c].dx;	//Move right
	if(kHeld & KEY_LEFT) 
		sprites[c].x -= sprites[c].dx;	//Move left
	if(kHeld & KEY_UP)
		sprites[c].y -= sprites[c].dy;	//Move up
	if(kHeld & KEY_DOWN)
		sprites[c].y += sprites[c].dy;	//Move down
	if(kHeld & KEY_START)
		return false;
	boundaries(c);
...
//---------------------------------------------------------------------------------
	...
	while (loop) {
		...
		loop = moveSprites();	//player moves their sprite. Most people just drink it
